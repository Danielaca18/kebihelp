...
SOURCES_FILE = "~/.config/kebihelp/sources.json"
...
class Config:
    ...
    def add_keybinding(self, tab, group, keybinding, description):
        print(
            "Importing keybinding to tab {}: Group {} - {} - {}".format(
                tab, group, keybinding, description
            )
        )
        ...
        if not group in self.config["Keybindings"][tab]:
            self.config["Keybindings"][tab][group] = {
                "_hidden": False,
                "keybindings": [],
            }
        ...
        for idx, k in enumerate(self.config["Keybindings"][tab][group]["keybindings"]):
            if k["value"] == keybinding:
                found = True
                self.config["Keybindings"][tab][group]["keybindings"][idx]["label"] = (
                    description
                )
        if not found:
            self.config["Keybindings"][tab][group]["keybindings"].append(
                {"value": keybinding, "label": description}
            )
