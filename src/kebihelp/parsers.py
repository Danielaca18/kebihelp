        ...
        for section in sections:
            ...
            for tr in trs:
                ...
                if len(tds) > 1:
                    ...
                    self.config.add_keybinding(
                        self.destination_tab, section, value, label
                    )
            ...
            if os.path.isfile(file):
                with open(file, "r") as file:
                    ...
                    for line in lines:
                        ...
                        if res:
                            tab = self.destination_tab
                            if self.config.config["Parameters"]["importer"].get(
                                "position_tab"
                            ):
                                tab = res.group(
                                    self.config.config["Parameters"]["importer"][
                                        "position_tab"
                                    ]
                                )
                            group = res.group(
                            ...
                            self.config.add_keybinding(tab, group, keybind, label)
